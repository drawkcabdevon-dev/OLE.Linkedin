"""
ADK Agent for Online Everywhere.
Wraps AI tools (content gen, research, trends) for deployment to
Vertex AI Agent Engine (uses promo credits).
"""
...
def chat_reply(user_message: str, recent_history: str = "") -> str:
    ...
    user_prompt = f"{context}User: {user_message}\n\nResponse:"
    try:
...
def build_agent():
    """Build and return the ADK agent with all tools."""
    import vertexai
    from google.adk.agents import LlmAgent
    from google.adk.tools import FunctionTool
    from vertexai.preview.reasoning_engines import AdkApp

    vertexai.init(project="linkedin-agent-501504", location="us-central1")
...
if __name__ == "__main__":
    app = build_agent()
    remote = app.deploy()
    print(f"Deployed! Resource: {remote.resource_name}")
