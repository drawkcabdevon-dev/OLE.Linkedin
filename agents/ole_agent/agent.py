...
# Use Vertex AI endpoint when running in GCP (Agent Engine), fall back to public API
PROJECT = os.getenv("GOOGLE_CLOUD_PROJECT", "")
LOCATION = os.getenv("GOOGLE_CLOUD_LOCATION", "us-central1")
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY", "")
if PROJECT:
    GEMINI_URL = f"https://{LOCATION}-aiplatform.googleapis.com/v1/projects/{PROJECT}/locations/{LOCATION}/publishers/google/models/{GEMINI_MODEL}:generateContent"
else:
...
def _call_gemini(system_prompt: str, user_prompt: str, temperature: float = 0.7) -> str:
    ...
    if PROJECT:
        ...
        creds, _ = default()
        creds.refresh(httpx.request("GET", "https://www.googleapis.com"))
